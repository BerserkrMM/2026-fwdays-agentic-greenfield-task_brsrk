...
#### Scenario: An empty or over-long account name is rejected

- **WHEN** the user submits an account name that is empty, only whitespace, or exceeds the supported length
- **THEN** the system rejects the action with an explicit message
- **AND** no account is created or renamed to an invalid name
