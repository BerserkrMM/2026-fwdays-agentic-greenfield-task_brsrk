...
The Foundation capability is the platform every other Finup capability builds on
and the TC-MOD-02 coordination owner of all shared / cross-cutting files and
schema. It provides the installable PWA app shell and global navigation, the
`fin-*` design-token ownership, the six shared screen states, the imports-hub
skeleton, the single shared Postgres client boundary with an in-memory fallback,
the framework-free domain contracts and module-port convention, and the owned
`input_events` / `parser_runs` / `ledger_items` schema plus the item-creation
contract. It ships the seams (boundaries, contracts, schema, shell) without
implementing downstream features.
