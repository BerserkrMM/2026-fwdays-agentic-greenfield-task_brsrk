...
for example `40 грн ковбаса, 20 грн хліб`. (FR-TEXT-01)
...
Submitted text SHALL be stored as an `input_event` with source `text` before
normalization or parsing. (FR-TEXT-02, FR-IMPORT-02, NFR-PRIV-02)
...
The stored text SHALL be source-normalized and passed to the AI parser to produce
ledger item drafts. (FR-TEXT-03, FR-PARSE-01)
...
- **THEN** text-specific normalization prepares the payload
...
`parser_run` for a later retry. (FR-TEXT-03, FR-PARSE-08, FR-ITEM-07)
