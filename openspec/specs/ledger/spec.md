...
#### Scenario: Archived-account items still count toward its per-account balance

- **GIVEN** an archived (soft-deleted) account with non-deleted ledger items
- **WHEN** the per-account balance query runs
- **THEN** the archived account's non-deleted items still contribute to its own balance
- **AND** the per-account grouping depends on the item's `account_id`, not on account archive state
...
#### Scenario: Category totals group by raw category text

- **WHEN** per-category totals are requested
- **THEN** the totals group by the raw `category` text stored on each item
- **AND** deleted items are excluded
- **AND** no separate category table join is required
