...
**Current state** — the landing-page commit was created, then the first `bash crash_course/run-all-checks.sh` exposed one expected regression in `src/app-pages.smoke.test.ts`: the home route assertion still expected `redirect:/dashboard`. Fixed it to expect `redirect:/about` and added an `/about` presentation-page smoke assertion so coverage does not drop from the large static page. The rerun of `bash crash_course/run-all-checks.sh` is green: **13/13 gates passed**. Coverage improved over baseline (lines/statements 69.05→73.41, functions 80.71→80.9, branches 90.08→90.37), so the ratchet passes without updating the baseline. `/about` remains a read-only presentation/support page for the course submission, not a new Finup MVP capability.

**Next steps** — commit the final smoke-test/handoff update, then owner can use `bash crash_course/run-all-checks.sh` during the demo. Optional: run with `--quick` for a shorter rehearsal or `--with-fallow` if an advisory structural audit should be shown.
