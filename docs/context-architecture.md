...
| Loaded when… | Source |
|---|---|
| working in a domain | `src/`/`app/` code + that domain's spec under `openspec/specs/<domain>/` |
| a reusable procedure applies | a `SKILL.md` skill (vendored under `.claude/skills/`, listed in `skills-lock.json`) |
...
- **Static budget:** 4k tokens. **Today:** `AGENTS.md` + `CLAUDE.md` are within budget.
- **Recently demoted to dynamic:** Next.js framework conventions → `node_modules/next/dist/docs/` (read on demand, never from memory).
- **Owning ADR:** [ADR-0002](adr/0002-context-architecture.md).
