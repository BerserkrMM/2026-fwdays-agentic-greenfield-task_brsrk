...
node scripts/check-red-green-evidence.mjs --slice <slice> --strict
node scripts/check-handoff-fresh.mjs
...
- [ ] Tests written FIRST from the spec and observed to FAIL (red), then made to
      ...
      test weakened to go green. Save durable evidence as
      `openspec/changes/<slice>/evidence/red-run.json` (non-zero exit,
      failingTests, gitHead, timestamp) and `green-run.json` (exit 0).
...
- [ ] Raw reviewer outputs persisted under `openspec/changes/<slice>/reviews/`
      and linked from `review-findings.json.rawEvidence`; summary-only review
      evidence is insufficient for strong maker≠checker claims.
- [ ] Review evidence persisted (`openspec/changes/<slice>/review-findings.json`,
      ...
      Note: this is only the deterministic trajectory half; if trajectory-eval
      was not run, summaries must say so and must not claim the "entire loop".
- [ ] No server action in the slice can 500 on user input; no silent external
      failures; forms keyed by server state.
- [ ] Change archived, `openspec list` shows no active changes, generated slice
      report produced with `node scripts/slice-report.mjs --slice <slice>`,
      `docs/current-state.md` updated LAST, and slice committed with
